#!/usr/bin/env node
/**
 * Idempotent Cloudflare bootstrap for billing.joed.dev:
 *   DNS:  billing.joed.dev → <tunnel-id>.cfargotunnel.com (proxied)
 *   Tunnel ingress: billing.joed.dev → http://traefik:80
 *
 * Env:
 ...
 *   BILLING_HOSTNAME      (optional; default: billing.joed.dev)
 *   BILLING_SERVICE_URL   (optional; default: http://traefik:80)
 *   DRY_RUN=1
 *
 * Usage:
...
const HOSTNAME = process.env.BILLING_HOSTNAME || `billing.${ZONE_NAME}`;
const SERVICE = process.env.BILLING_SERVICE_URL || "http://traefik:80";
...
async function ensureDns(zoneId) {
  ...
  const create = {
    ...
    content: target,
    proxied: true,
    comment: "WL-Universe site-billing",
  };
  ...
  if (!TOKEN) {
    console.error(
      "CLOUDFLARE_API_TOKEN is required for programmatic setup.\n" +
        ...
        "  Subdomain: billing  Domain: joed.dev  Service: http://traefik:80",
    );
    process.exit(2);
  }
