...
| `wl-status.joed.dev` | Site DNS/liveness dashboard (`/opt/services/apps/wl-status`) |

**Status page note:** Feature-deployment listing belongs on **`wl-status.joed.dev`** (or a new path there). Do not overload Dozzle at `status.joed.dev` unless the owner explicitly wants a rename/move later.
...
## wl-status integration

Extend `/opt/services/build/wl-status` (and keep config in-repo if possible):
...
- Owner said `status.joed.dev`; implement on **`wl-status.joed.dev`** and note Dozzle collision in the PR. Optional follow-up: redirect or rename.
...
| 7 | wl-status: Feature deployments list + active.json registry |
